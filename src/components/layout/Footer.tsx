    ...
    <footer style={{ background: 'rgba(0,0,0,0.3)', borderTop: '1px solid var(--glass-border)', padding: '60px 6% 32px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '48px', marginBottom: '48px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
            {[
              ...
              <a key={s.href} href={s.href} target="_blank" rel="noopener"
                style={{ width: '36px', height: '36px', borderRadius: '9px', background: 'var(--glass)', border: '1px solid var(--glass-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', textDecoration: 'none', fontSize: '0.7rem', fontWeight: 700, transition: 'all 0.2s' }}
        ...
        <div>
          ...
          {['Marketing Strategy','Social Media','Paid Advertising','Branding','Content Creation','Copywriting','Web Development'].map(s => (
            <Link key={s} href={`/services/${s.toLowerCase().replace(/\s+/g,'-')}`}
              style={{ display: 'block', fontSize: '0.78rem', color: 'var(--text-muted)', textDecoration: 'none', marginBottom: '8px', transition: 'color 0.2s' }}
        ...
        <div>
          ...
          {[['Home','/'],['About','/about'],['Packages','/packages'],['Blog','/blog'],['Careers','/careers'],['Affiliate','/affiliate'],['Privacy','/privacy']].map(([l,h]) => (
            <Link key={h} href={h}
              style={{ display: 'block', fontSize: '0.78rem', color: 'var(--text-muted)', textDecoration: 'none', marginBottom: '8px', transition: 'color 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.color = 'var(--white)'}
              onMouseLeave={e => e.currentTarget.style.color = 'var(--text-muted)'}>
        ...
        <div>
          ...
          <a href="https://wa.me/447376441603" target="_blank" rel="noopener" style={{ display: 'block', fontSize: '0.78rem', color: 'var(--text-muted)', textDecoration: 'none', marginBottom: '8px' }}>WhatsApp</a>
          <a href="mailto:hello@yallagrow.net" style={{ display: 'block', fontSize: '0.78rem', color: 'var(--text-muted)', textDecoration: 'none', marginBottom: '8px' }}>hello@yallagrow.net</a>
          <Link href="/contact" style={{ display: 'block', fontSize: '0.78rem', color: 'var(--sky)', textDecoration: 'none', marginBottom: '8px' }}>Contact Form →</Link>
        </div>
      </div>

      <div style={{ borderTop: '1px solid var(--glass-border)', paddingTop: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
        <p style={{ fontSize: '0.72rem', color: 'var(--text-dim)' }}>© 2025 YallaGrow Marketing Agency · All rights reserved.</p>
        <Link href="/privacy" style={{ fontSize: '0.72rem', color: 'var(--text-dim)', textDecoration: 'none' }}>Privacy Policy</Link>
      </div>
    </footer>
  )
}
