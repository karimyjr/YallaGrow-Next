    ...
    <div style={{ paddingTop: '80px' }}>
      ...
      <div style={{ padding: '80px 6% 60px', textAlign: 'center' }}>
        ...
        <h1 className="section-title" style={{ margin: '12px auto 16px' }}>
          Simple. Transparent.<br />Built for Real Businesses.
        </h1>
        <p className="section-sub" style={{ margin: '0 auto 32px' }}>
          No hidden fees. No long-term lock-ins. No inflated retainers.
        </p>
        ...
        <div style={{ marginTop: '20px' }}>
          <a href="/quiz" className="btn-secondary" style={{ fontSize: '0.82rem', textDecoration: 'none' }}>
            🎯 Not sure which plan? Find yours in 2 min →
          </a>
        </div>
      </div>
      ...
      {/* Loading skeleton */}
      {loading ? (
        <div style={{ padding: '0 6% 80px', textAlign: 'center', color: 'var(--text-dim)', fontSize: '0.85rem' }}>
          Loading packages...
        </div>
      ) : (
            ...
            <div
              ...
              style={{
                background: p.featured
                  ? 'linear-gradient(135deg, rgba(1,32,76,0.6), rgba(16,161,219,0.08))'
                  : 'rgba(249,253,254,0.02)',
              ...
              onMouseEnter={e => {
                if (!p.featured) e.currentTarget.style.borderColor = 'rgba(16,161,219,0.2)'
                e.currentTarget.style.transform = 'translateY(-4px)'
              }}
              onMouseLeave={e => {
                if (!p.featured) e.currentTarget.style.borderColor = 'var(--glass-border)'
            ...
            >
              {p.featured && (
                <div style={{
                  ...
                  color: '#fff', fontSize: '0.62rem', fontWeight: 800, letterSpacing: '1px',
              ...
              <div style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--sky)', marginBottom: '6px' }}>
                {p.tier}
              </div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.5rem', color: 'var(--white)', marginBottom: '4px' }}>
                {p.tier}
              </div>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '4px', fontStyle: 'italic' }}>
                {p.subtitle}
              </div>
              <div style={{ fontSize: '0.76rem', color: 'var(--text-muted)', marginBottom: '24px', lineHeight: 1.5, fontWeight: 300 }}>
                {p.for}
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '2px', marginBottom: '28px', paddingBottom: '24px', borderBottom: '1px solid var(--glass-border)' }}>
                ...
                <span style={{ fontFamily: 'Syne, sans-serif', fontSize: '3rem', fontWeight: 800, color: 'var(--white)', letterSpacing: '-2px', lineHeight: 1 }}>
                  {p.price}
                </span>
              ...
              <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px', flex: 1, marginBottom: '28px', padding: 0 }}>
                {p.feats.map((f, fi) => (
                  <li key={fi} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', fontSize: '0.82rem', color: 'rgba(249,253,254,0.65)', lineHeight: 1.5, fontWeight: 300 }}>
                    <span style={{ color: 'var(--sky)', flexShrink: 0, fontWeight: 700, marginTop: '1px' }}>✓</span>
                    {f}
                  </li>
                ))}
              </ul>
              <a
                ...
                rel="noopener"
                style={{
                  width: '100%', padding: '14px', borderRadius: '10px', fontSize: '0.85rem',
                  fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif',
                  transition: 'all 0.25s', textAlign: 'center', textDecoration: 'none', display: 'block',
                  background: p.featured ? 'var(--sky)' : 'transparent',
                  color: p.featured ? 'var(--dark)' : 'var(--sky)',
                  border: p.featured ? '1px solid var(--sky)' : '1px solid rgba(16,161,219,0.3)',
                }}
              >
